...
static int frameBufferAllocator[PERIPHERAL_MAX_FRAMEBUFFERS];
...
void gfx_init()
{
  ...
  for (int i = 1; i < PERIPHERAL_MAX_FRAMEBUFFERS; i++) {
    frameBufferAllocator[i] = -1;
  }
}

unsigned gfx_frameFrameBufferDirty(unsigned fb)
{
...
void gfx_setFrameFrameBufferDirty(unsigned fb)
{
  frameBufferDirty[fb] = 1;
}

void gfx_setVideoResolution(unsigned fb, unsigned short x, unsigned short y, unsigned scaling)
{
  if (fb == 0) {
    videoWidth = x;
    videoHeight = y;
...
unsigned gfx_createFrameBuffer(unsigned w, unsigned h)
{
  ...
  for (int i = 1; i < PERIPHERAL_MAX_FRAMEBUFFERS; i++) {
    if (frameBufferAllocator[i] == -1) {
      frameBufferAllocator[i] = kernel_getPid();
      fb = i;
      break;
    }
  }
...
void gfx_drawRGBA(unsigned fb, unsigned short x, unsigned short y, unsigned *ptr)
{
  dirty();
...
unsigned gfx_createFrameBufferFromData(unsigned w, unsigned h, unsigned* ptr)
{
  unsigned short x, y;
...
void gfx_putData(unsigned fb, int x, int y, unsigned byte)
{
  peripheral.video[fb].videoAddressX = x;
  peripheral.video[fb].videoAddressY = y;
...
void gfx_fputData(int x, int y, unsigned byte)
{
...
void gfx_bitBltEx(unsigned fb, short sx, short sy, short dx, short dy, unsigned short sw, unsigned short sh, unsigned short dw, unsigned short dh, unsigned src)
{
  dirty();
...
void gfx_drawLine(unsigned fb, int x0, int y0, int x1, int y1, unsigned color) {
...
void gfx_drawCharRetro(unsigned fb, int x, int y, char c, unsigned color, unsigned char size) {
...
void gfx_drawString(unsigned fb, int x,  int y, char *c, unsigned  color) {
...
void gfx_alpha(unsigned fb, unsigned alpha) {
